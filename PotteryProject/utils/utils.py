...
def index_points_pc(points, idx):
    """
    Input:
...
def farthest_point_sample_pc(xyz, npoint, RAN=True):
    """
    Input:
        xyz: pointcloud data, [N, C]
        npoint: number of samples
        RAN: flag to determine the initial random point selection
    Return:
    ...
    if RAN:
        ...
        farthest = torch.randint(1, N, (1,), dtype=torch.long).to(device)
    
    for i in range(npoint):
        centroids[i] = farthest
        centroid = xyz[farthest, :].view(1, C)
        dist = torch.sum((xyz - centroid) ** 2, -1)
        mask = dist < distance
        distance[mask] = dist[mask]
        farthest = torch.max(distance, -1)[1]
